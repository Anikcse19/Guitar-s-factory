...
const MyOrders = () => {
    ...
    useEffect(() => {
        fetch(`https://quiet-depths-28219.herokuapp.com/purchaser?email=${user.email}`)
            .then(res => res.json())
            .then(data => {
                setOrders(data)
            })
                    ...
                    <TableBody>
                        {orders.map((order) => (
                            ...
                            >
                                ...
                                <TableCell align="right"><Button onClick={() => handleDelete(order._id)}>Cancel</Button></TableCell>
